...
async function generateExcuse(input, mode) {
  const prompt = `次の状況における雅な言い訳を生成して。前置き、空白は要りません。雅な言い訳の文章とその意味のみ生成して。ただし、文章と意味の間には、"|"を入れてください。もし、今の状況と関係のない文章が送られてきた場合は会話として返してください。今の状況と生成したい雅な言い訳の口調と文章の長さ: ${input}。`;
